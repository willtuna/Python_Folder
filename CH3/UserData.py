...
#                              namedtuple name,  data member name delimited by name
def main():
    if len(sys.argv) == 1 or sys.argv[1] in {'-h','--help'}:
        print('uasge: {0} file1 [file2 ...'.format(sys.argv[0]))
        sys.exit()

    usernames = set() # empty set
    users ={} #empty dict
    for filename in sys.argv[1:]:
        for line in open(filename, encoding='utf8'):
            line = line.rstrip()
            if line: # process non empty line
...
def process_line(line,usernames):
    ...
    user = User(username , fields[FORENAME],fields[MIDDLENAME],fields[SURNAME],fields[ID])# create a nametuple
...
def generate_username(fields,usernames):
    ...
    count = 1
    while username in usernames:# avoid same username but different person
